...
class PredictionHandler(RequestHandler):
    def initialize(self, serving_host, serving_port, model_name):
        ...
        self.channel = grpc.insecure_channel(
            "{}:{}".format(self.serving_host, self.serving_port)
        )
    ...
    def get(self):
        ...
        ].CopyFrom(tf.compat.v1.contrib.util.make_tensor_proto([[1]], shape=[1, 1]))

        try:
            result_future = yield fwrap(self.stub.Predict.future(request, 10))
            self.set_status(200)
            response = {
                "outputs": np.array(result_future.outputs["outputs"].int_val).tolist()
            }
